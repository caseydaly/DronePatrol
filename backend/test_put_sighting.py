...
class TestClass:
    ...
    def test_good_data(self):
        response = self.client.post('/api/sighting', self.insert_obj)
        assert response.status_code == 200
        cursor = self.db.cursor()
        data = cursor.execute("SELECT * FROM Sightings WHERE SightingDate='2020-12-03 09:09:00' and Latitude=37.75001907348633 and Longitude=-122.51163482666016; ")
        ...
        for obj in results:
            temp = {
                ...
                "img": self.client.get_img_from_path(obj[3]) if obj[3] is not None else obj[3],
            ...
            print(temp["location"])
            if (self.compare_all_but_img(temp, self.ref_obj)):
                found = True
        assert found